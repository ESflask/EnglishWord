...
AIエージェントは、ユーザーから明示的に指示されない限り、以下のファイルを `git add` / `git commit` / `git push` してはならない。

### 絶対にコミット・プッシュしないもの
| ファイル / パターン | 理由 |
|---|---|
| `GoogleService-Info.plist` | Firebase の API キー・認証情報 |
| `*.env`, `.env.*` | 環境変数・シークレット |
| `*.key`, `*.pem`, `*.p12`, `*.p8` | 秘密鍵・証明書 |
| `secrets.*`, `credentials.*` | 認証情報全般 |
| `.claude/` | Claude Code のローカル設定（パーミッション情報） |
| `.gemini/` | Gemini のローカル設定 |

### ユーザーの確認を取ってからコミットするもの
| ファイル / パターン | 理由 |
|---|---|
| `PROGRESS.md` 等の作業ログ | 内部メモであり公開不要な場合がある |
| `add_*.py` 等の一時スクリプト | 作業用の使い捨てファイルで不要なことが多い |
| `existing_*.json` 等の中間データ | 生成過程の一時データ |

### リポジトリの可視性変更について
- `gh repo edit --visibility public` など、リポジトリを public にする操作は **必ずユーザーの明示的な許可を得てから** 実行すること。
- public 化の前に、上記の禁止ファイルが git 履歴（`git log --all --name-only`）に含まれていないことを確認すること。
